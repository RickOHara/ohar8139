...
public class AStarSearch  {
	...
	private static final int numGraphNodes = 100;
	 ...
	 * 
	 */
	public static Graph createGraphToGoalWithBeacons(Toroidal2DPhysics state, Ship myShip, Position goalPosition, Random random) {
		// where am I?
		...
		for (Vertex vertex1 : graph.getVertices()) {
			for (Vertex vertex2 : graph.getVertices()) {
				...
				if (distance > 0 && distance < maxNodeDistance && 
						isFreeLine(vertex1.getPosition(), vertex2.getPosition(), state)) {
